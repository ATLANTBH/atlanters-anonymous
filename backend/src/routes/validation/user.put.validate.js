...
export default async (User, user, reqUser) => {
  ...
  if(email) {
    if(email === user.email) throw new Error(`Email matches the one currently in use`);
    await User.checkEmailValid(reqUser.email);
  }
  if(password) {
    User.checkPasswordValid(reqUser.password);
    if(await compare(password, user.password)) throw new Error(`Password matches the one currently in use`)
