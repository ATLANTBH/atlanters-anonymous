...
describe('Goes through sign up, poll template creation, poll creation, deletion and sign out', () => {
  ...
  test('Sign up', done => {
    ...
    request(expressApp)
      ...
      .then(async response => {
        ...
        expect(
          await app.models.User.isPasswordHashProper(
            user.password,
            body.password
          )
        ).toBe(true);
  ...
  test('Get poll template', done => {
    ...
    request(expressApp)
      .get('/poll-templates/' + pollTemplateId)
      .set('x-auth', token)
      .then(response => {
        expect(response.statusCode).toBe(200);
        expect(typeof response.body).toBe('object');
  ...
  test('Submit answer', done => {
    ...
    request(expressApp)
      .post(
        '/poll-templates/' + pollTemplateId + '/poll/' + pollId + '/answers'
      )
      .send(answer)
      .set('x-auth', token)
      .set('Accept', 'application/json')
  ...
  test('Delete poll template', done => {
    request(expressApp)
      .delete('/poll-templates/' + pollTemplateId)
      .set('x-auth', token)
      .then(response => {
        expect(response.statusCode).toBe(200);
        done();
