...
function getResHeaderMock() {
  ...
  const header = {
    send: resMock
      .expects('send')
      .once()
      .withExactArgs(output),
  };
  const res = {
    header: headerMock
      .expects('header')
      .once()
      .withExactArgs('x-auth', '')
      .returns(resData),
    cookie: cookieMock
      .expects('cookie')
      .once()
      .withExactArgs('user_id', output.id, {
        httpOnly: true,
        secured: true,
        signed: true,
      }),
...
function getSignOutResUserMock() {
  ...
  const user = {
    removeAuthenticationToken: userMock
      .expects('removeAuthenticationToken')
      .once()
      .withExactArgs(''),
  };
...
describe('Auth Unit Tests', () => {
  ...
  describe('GET', () => {
    test('sign-up', async () => {
      ...
      User.insert
        .once()
        .withExactArgs(input.body)
        .returns({ user: output, token: '' });
    ...
    test('sign-in', async () => {
      ...
      User.authenticate
        .once()
        .withExactArgs(input.body.email, input.body.password)
        .returns(output);
